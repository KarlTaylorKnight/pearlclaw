...
- **SecurityStub field list is exactly the requested six fields:** `workspace_dir`, `rate_limited`, `action_budget`, `allow_absolute_under_root`, `allow_resolved_outside_workspace`, `extra_blocked_paths`, plus methods only. No extra policy fields were added. (The Phase 7-D brief miscounted these as seven; the field list was the source of truth — corrected here in Phase 7-D.1.)
