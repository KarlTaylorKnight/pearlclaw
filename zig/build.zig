...
pub fn build(b: *std.Build) void {
    ...
    const eval_memory_tools_mod = b.createModule(.{
        .root_source_file = b.path("src/tools/eval_memory_tools.zig"),
        .target = target,
        .optimize = optimize,
    });
    eval_memory_tools_mod.addImport("zeroclaw", zeroclaw_mod);
    const eval_memory_tools_exe = b.addExecutable(.{
        .name = "eval-memory-tools",
        .root_module = eval_memory_tools_mod,
    });
    linkSqlite(b, eval_memory_tools_exe);
    b.installArtifact(eval_memory_tools_exe);

    const eval_file_tools_mod = b.createModule(.{
        .root_source_file = b.path("src/tools/eval_file_tools.zig"),
        .target = target,
        .optimize = optimize,
    });
    eval_file_tools_mod.addImport("zeroclaw", zeroclaw_mod);
    const eval_file_tools_exe = b.addExecutable(.{
        .name = "eval-file-tools",
        .root_module = eval_file_tools_mod,
    });
    linkSqlite(b, eval_file_tools_exe);
    b.installArtifact(eval_file_tools_exe);

    const eval_content_search_mod = b.createModule(.{
        .root_source_file = b.path("src/tools/eval_content_search.zig"),
        .target = target,
        .optimize = optimize,
    });
    eval_content_search_mod.addImport("zeroclaw", zeroclaw_mod);
    const eval_content_search_exe = b.addExecutable(.{
        .name = "eval-content-search",
        .root_module = eval_content_search_mod,
    });
    ...
    const eval_hardware_memory_map_mod = b.createModule(.{
        .root_source_file = b.path("src/tools/eval_hardware_memory_map.zig"),
        .target = target,
        .optimize = optimize,
    });
    eval_hardware_memory_map_mod.addImport("zeroclaw", zeroclaw_mod);
    const eval_hardware_memory_map_exe = b.addExecutable(.{
        .name = "eval-hardware-memory-map",
        .root_module = eval_hardware_memory_map_mod,
    });
    linkSqlite(b, eval_hardware_memory_map_exe);
    b.installArtifact(eval_hardware_memory_map_exe);

    const eval_hardware_board_info_mod = b.createModule(.{
        .root_source_file = b.path("src/tools/eval_hardware_board_info.zig"),
        .target = target,
        .optimize = optimize,
    });
    eval_hardware_board_info_mod.addImport("zeroclaw", zeroclaw_mod);
    const eval_hardware_board_info_exe = b.addExecutable(.{
        .name = "eval-hardware-board-info",
        .root_module = eval_hardware_board_info_mod,
    });
