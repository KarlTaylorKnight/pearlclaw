...
/// Provider DTOs are defined in `providers/types.zig` (Phase 3-F.2); these
/// aliases preserve the historical `dispatcher.X` import paths used across
/// the codebase. Future commits can migrate consumers to import from
/// `providers/types.zig` directly.
pub const ToolCall = provider_types.ToolCall;
pub const TokenUsage = provider_types.TokenUsage;
pub const ChatResponse = provider_types.ChatResponse;
pub const ChatMessage = provider_types.ChatMessage;
pub const ToolResultMessage = provider_types.ToolResultMessage;
pub const AssistantToolCallsMessage = provider_types.AssistantToolCallsMessage;
pub const ConversationMessage = provider_types.ConversationMessage;

/// Result of executing a tool, fed to format_results. Stays in dispatcher
/// because it is the dispatcher's intermediate value, not a provider DTO.
