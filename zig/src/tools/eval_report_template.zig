...
//!   {"op":"execute","tool":"report_template","args":{...}}
//!
//! No filesystem or binary setup is required — the report template tool
//! is pure-compute. Each input line is parsed, dispatched to the Zig
//! ReportTemplateTool, and the canonical eval result line is written to
//! stdout.
...
fn runOp(allocator: std.mem.Allocator, line: []const u8, writer: anytype) !void {
    ...
    if (!std.mem.eql(u8, tool_name, "report_template")) return EvalError.InvalidScenario;
    const args = getField(parsed.value, "args") orelse return EvalError.InvalidScenario;

    var tool_impl = agent_tools.ReportTemplateTool.init(allocator);
