...
fn runOp(allocator: std.mem.Allocator, line: []const u8, writer: anytype) !void {
    ...
    if (getString(setup, "workspace") orelse getString(setup, "home")) |workspace| {
        try std.posix.chdir(workspace);
    }

    var result = try executeTool(allocator, tool_name, args);
...
fn executeTool(
    ...
    if (std.mem.eql(u8, tool_name, "file_write")) {
        var tool_impl = agent_tools.FileWriteTool.init(allocator);
        defer tool_impl.deinit(allocator);
        return tool_impl.tool().execute(allocator, args);
    }
    if (std.mem.eql(u8, tool_name, "file_edit")) {
        var tool_impl = agent_tools.FileEditTool.init(allocator);
        defer tool_impl.deinit(allocator);
        return tool_impl.tool().execute(allocator, args);
    }
    if (std.mem.eql(u8, tool_name, "glob_search")) {
        var tool_impl = agent_tools.GlobSearchTool.init(allocator);
        defer tool_impl.deinit(allocator);
        return tool_impl.tool().execute(allocator, args);
    }
...
fn applySetup(allocator: std.mem.Allocator, setup: std.json.Value) !void {
    ...
    if (getField(setup, "symlinks")) |symlinks| {
        if (symlinks != .array) return EvalError.InvalidScenario;
        for (symlinks.array.items) |link| {
            if (link != .object) return EvalError.InvalidScenario;
            const from = getString(link, "from") orelse return EvalError.InvalidScenario;
            const to = getString(link, "to") orelse return EvalError.InvalidScenario;
            if (std.fs.path.dirname(from)) |parent| try std.fs.cwd().makePath(parent);
            if (std.fs.path.isAbsolute(from)) {
                try std.fs.symLinkAbsolute(to, from, .{});
            } else {
                try std.fs.cwd().symLink(to, from, .{});
            }
        }
    }
}
