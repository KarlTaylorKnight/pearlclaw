//! eval-image-info - offline image_info parity runner.
...
fn runOp(allocator: std.mem.Allocator, line: []const u8, writer: anytype) !void {
    ...
    if (!std.mem.eql(u8, tool_name, "image_info")) return EvalError.InvalidScenario;
    ...
    const security = agent_tools.security_stub.SecurityStub{
    ...
    var tool_impl = agent_tools.ImageInfoTool.initWithSecurity(security);
...
fn applySetup(allocator: std.mem.Allocator, setup: std.json.Value) !void {
    ...
    if (getField(setup, "binary_files")) |binary_files| {
        if (binary_files != .object) return EvalError.InvalidScenario;
        var it = binary_files.object.iterator();
        while (it.next()) |entry| {
            if (entry.value_ptr.* != .string) return EvalError.InvalidScenario;
            const path = entry.key_ptr.*;
            const encoded = entry.value_ptr.string;
            const decoded_len = std.base64.standard.Decoder.calcSizeForSlice(encoded) catch return EvalError.InvalidScenario;
            const bytes = try allocator.alloc(u8, decoded_len);
            defer allocator.free(bytes);
            std.base64.standard.Decoder.decode(bytes, encoded) catch return EvalError.InvalidScenario;
