//! eval-hardware-board-info — offline hardware_board_info parity runner.
//!
//! Scenario shape (JSONL, one op per line):
//!   {"op":"execute","tool":"hardware_board_info","setup":{"boards":[...]},"args":{...}}
//!
//! No filesystem setup is required — pure static datasheet lookup.
...
fn runOp(allocator: std.mem.Allocator, line: []const u8, writer: anytype) !void {
    ...
    if (!std.mem.eql(u8, tool_name, "hardware_board_info")) return EvalError.InvalidScenario;
    ...
    var tool_impl = agent_tools.HardwareBoardInfoTool.init(allocator, boards);
