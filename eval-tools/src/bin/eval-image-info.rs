...
use zeroclaw_tools::image_info::ImageInfoTool;
...
async fn main() -> Result<()> {
    ...
    for raw_line in input.lines() {
        ...
        if op != "execute" {
            anyhow::bail!("unknown image_info op: {op}");
        }

        let tool = required_string(&value, "tool")?;
        if tool != "image_info" {
            anyhow::bail!("unknown image_info tool: {tool}");
        }
...
async fn execute_tool(security: Arc<SecurityPolicy>, args: Value) -> ToolResult {
    match ImageInfoTool::new(security).execute(args).await {
        Ok(result) => result,
        Err(err) => ToolResult {
            success: false,
            output: String::new(),
...
fn apply_setup(setup: &Value) -> Result<()> {
    ...
    if let Some(binary_files) = setup.get("binary_files") {
        let binary_files = binary_files
            .as_object()
            .context("setup binary_files must be an object")?;
        for (path, encoded) in binary_files {
            let encoded = encoded
                .as_str()
                .context("setup binary_files values must be base64 strings")?;
            let bytes = base64::engine::general_purpose::STANDARD
                .decode(encoded)
                .context("invalid setup binary_files base64")?;
