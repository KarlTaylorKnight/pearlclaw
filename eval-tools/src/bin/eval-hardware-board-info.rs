...
use zeroclaw_tools::hardware_board_info::HardwareBoardInfoTool;
...
async fn main() -> Result<()> {
    ...
    for raw_line in input.lines() {
        ...
        if op != "execute" {
            anyhow::bail!("unknown hardware_board_info op: {op}");
        }

        let tool_name = required_string(&value, "tool")?;
        if tool_name != "hardware_board_info" {
        ...
        let result = HardwareBoardInfoTool::new(boards).execute(args).await?;
